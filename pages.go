// Source: "github.com/qiangxue/go-rest-api/pkg/pagination"
...
func (p *Pages) Offset() uint {
	return uint((p.Page - 1) * p.PerPage)
}

// Limit returns the LIMIT value that can be used in a SQL statement.
func (p *Pages) Limit() uint {
	return uint(p.PerPage)
}
